...
export default class MyChart {
    ...
    update() {
		...
		let value1Msg = (0).toLocaleString(undefined, compactNumberFormatter);
		let value2Msg = '';
		let value1TooltipMsg = '';
		let value2TooltipMsg = '';
		let color = '#000000';
		...
		if (this.data.DataQueries && this.data.DataQueries[0].Series[0]) {
			...
			let value1 = Math.trunc(total1*100)/100;
			value1Msg = value1.toLocaleString(undefined, compactNumberFormatter);
			value1TooltipMsg = value1.toLocaleString(undefined, numberFormatter);
		}
		// calculate the totals of the second query
		if (this.data.Benchmark.DataQueries && this.data.Benchmark.DataQueries[0].Series[0] && benchmarkSet[0]) {
			...
			value2Msg = value2.toLocaleString(undefined, compactNumberFormatter);
			value2TooltipMsg = value2.toLocaleString(undefined, numberFormatter);
		}
